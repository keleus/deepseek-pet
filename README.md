...
- 思考、回复或交互等待时，在角色上方显示当前会话内容；
- 在角色下方用圆弧排列活跃会话，第一项为聚焦会话并以亮边标记，点击即可切换；
- 多个会话并行执行时进入“忙疯了”状态；
- 根据分析、回答、编码、网络搜索、子 Agent、完成和失败等场景切换表情；
...
- 鼠标移入角色时显示快捷图标，可查看当前会话上下文和今日活跃会话的 token 消耗；
- 支持拖动角色、单击互动、双击折叠，并记住角色位置；
...
- 双击角色：折叠为右下角小图标；
- 点击活跃会话：将该会话切换为聚焦会话；
- 鼠标移入角色后点击“上下文”图标：Pet 会通过对话气泡报告当前上下文窗口占用；
- 鼠标移入角色后点击“今日消耗”图标：Pet 会通过对话气泡报告今天有活动的会话累计 token；
- 出现批准或提问卡片时：可直接选择、输入、允许或拒绝。

统计值取自当前可用数据；模型尚未上报时会显示“暂无统计”。
